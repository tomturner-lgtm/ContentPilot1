...
export async function POST(req: Request) {
    try {
        ...
        if (!authUser) {
            console.error('❌ Tentative d\'annulation sans authentification')
        ...
        // 🔒 SÉCURITÉ : Vérifier que l'utilisateur ne peut annuler que SON propre abonnement
        ...
        if (!userCheck) {
            console.error('❌ Tentative d\'annulation d\'un abonnement d\'un autre utilisateur')
            return NextResponse.json(
                { error: 'Non autorisé' },
                { status: 403 }
            )
